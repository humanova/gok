...
func ExtractViewpoints(entries []model.Entry, maxClusters int) []model.Viewpoint {
	...
	for _, e := range entries {
		if e.Embedding != nil && len(e.Embedding.Slice()) > 0 {
			hasEmbeddings = true
			break
		}
	}
...
func clusterByEmbedding(entries []model.Entry, k int) []model.Viewpoint {
	...
	for _, e := range entries {
		...
		vec := e.Embedding.Slice()
		if len(vec) == 0 {
			continue
		}

		bestCluster := -1
		bestSim := float32(0.7) // minimum similarity threshold to join a cluster
		...
		if bestCluster >= 0 {
			...
			n := float32(len(clusters[bestCluster].members))
